...
class NAMLModel(BaseModel):
    ...
    def _build_newsencoder(self, embedding_layer, entity_embedding_layer):
        ...
        Args:
            ...
            entity_embedding_layer: an entity embedding layer
        Return:
            object: the news encoder of NAML.
        """
        hparams = self.hparams
        input_title_body_verts = keras.Input(
            shape=(hparams.title_size + hparams.body_size + 2 * hparams.entity_size + 2,), dtype="int32"
        )
        ...
        entity_title_repr = self._build_entity_titleencoder(entity_embedding_layer)(sequences_entity_input_title)
        ...
        entity_ab_repr = self._build_entity_abencoder(entity_embedding_layer)(sequences_entity_input_ab)
    ...
    def _build_entity_titleencoder(self, entity_embedding_layer):
        hparams = self.hparams
        ...
        model = keras.Model(sequences_input_entity, pred_title, name="entity_title_encoder")
        return model
    ...
    def _build_entity_abencoder(self, entity_embedding_layer):
        hparams = self.hparams
        sequences_input_entity = keras.Input(shape=(hparams.entity_size,), dtype='int32')
        embedded_sequences_entity = entity_embedding_layer(sequences_input_entity)
        y = layers.Dropout(hparams.dropout)(embedded_sequences_entity)
        ...
        model = keras.Model(sequences_input_entity, pred_title, name="entity_ab_encoder")
        return model
